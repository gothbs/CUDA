...
__global__ void SobelCUDA(const unsigned char *input, unsigned char *output, int width, int height, int bpp) {
    // Pour accéder à la mémoire partagée, il faut d'abord la déclarer
    ...
    if (x < width && y < height) {
        for (int c = 0; c < bpp; ++c) {
            sharedData[sharedIndex * bpp + c] = input[index + c];
        }
    }
    __syncthreads();
    ...
    if (x < width && y < height) {
        ...
        if (x >= 1 && y >= 1 && x + 1 < width && y + 1 < height) {
            for (int c = 0; c < bpp; ++c) {
                ...
                for (int ky = -1; ky <= 1; ++ky) {
                    for (int kx = -1; kx <= 1; ++kx) {
                        int pixel = sharedData[((threadIdx.y + ky) * blockDim.x + (threadIdx.x + kx)) * bpp + c];
            ...
            for (int c = 0; c < bpp; ++c) {
                output[index + c] = sharedData[sharedIndex * bpp + c];
            }
        }
    }
}
...
void SobelGPU(unsigned char *donnees, unsigned char *nouvellesDonnees, int largeur, int hauteur, int bpp, int iterations, int blockSizeX, int blockSizeY) {
    ...
    for (int i = 0; i < iterations; ++i) {
      SobelCUDA<<<gridDim, blockDim, blockSizeX * blockSizeY * bpp>>>(donneesSrcDevice, donneesDstDevice, largeur, hauteur, bpp);
